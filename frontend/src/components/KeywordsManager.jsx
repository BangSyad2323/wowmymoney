...
export default function KeywordsManager({ apiBaseUrl }) {
  const BASE = apiBaseUrl.replace('/api/transactions', '');
  ...
  return (
    <div className="glass-card">
      <div className="flex items-center gap-2 mb-5">
        ...
        <h3 className="text-lg font-semibold text-slate-800">Kelola Kata Kunci</h3>
      </div>

      {/* Add Form */}
      <form onSubmit={handleAdd} className="flex gap-2 mb-5 flex-wrap sm:flex-nowrap">
        <input
          type="text"
          value={newKeyword}
      ...
      {error && (
        <p className="text-sm text-rose-600 bg-rose-50 rounded-lg px-3 py-2 mb-4">{error}</p>
      )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Income Keywords */}
          <div>
            <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-2">✅ Pemasukan ({incomeKws.length})</p>
            <div className="flex flex-wrap gap-1.5">
              ...
              {incomeKws.length === 0 && <span className="text-xs text-slate-400">Belum ada</span>}
            </div>
          </div>

          {/* Expense Keywords */}
          <div>
            <p className="text-xs font-semibold text-rose-700 uppercase tracking-wide mb-2">💸 Pengeluaran ({expenseKws.length})</p>
            <div className="flex flex-wrap gap-1.5">
              {expenseKws.map(k => (
                <span
                  key={k.id}
              ...
              {expenseKws.length === 0 && <span className="text-xs text-slate-400">Belum ada</span>}
            </div>
          </div>
        </div>
      )}
